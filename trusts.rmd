...
Per model, the graphs of *fig. 5* summarise each history window's MAE per epoch. The CNN MAE values are spread-out, whereas the LSTM MAE values are compact.
...
The loss values distribution patterns (*fig. 6*) are similar to the MAE values distribution pattern.

\vspace{10pt}

Altogether, it seems

* It is possible to predict future patient admissions via LSTM and the predictors listed within the feature engineering sectio.
...
An option that would have improved the model is boosting.
