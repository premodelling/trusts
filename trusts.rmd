...
In brief, the aforementioned PHE England data set outlines the number of patients that each MSOA contributes to each NHS Trust each year; each MSOA contributes patients to one or more NHS Trusts.  However, each MSOA region is a member of exactly one LTLA region, hence the number of patients that each LTLA contributes to a NHS Trust, per year, is deducible via appropriate aggregates.  Hence, *the flow weight from a LTLA to a NHS Trust it contributes to, and for a specified year, is*
...
\begin{center}
    ...
    \end{tabular}
    \captionof{table}{Flow formula variables.  The population values are ONS estimates w.r.t. a specified year.}
\end{center}
...
```{r out.width = '85%', fig.align = 'center', fig.cap = 'Pending'}
